...
export async function execute(
  ...
  try {
    ...
    const displayName =
      guildMember?.displayName ?? targetUser.displayName ?? targetUser.username
    const avatarUrl =
      guildMember?.displayAvatarURL() ?? targetUser.displayAvatarURL()
    ...
    logger.info(
      `Stats displayed for ${stats.username} by ${interaction.user.username}`
    )
  } catch (error) {
