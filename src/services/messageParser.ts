...
import {
  DailyResults,
  UnresolvedDailyResults,
  ParsedPlayerResult,
  UnresolvedPlayerResult
} from '../types'
 ...
 */
export function isWordleMessage(
  message: Message,
...
function parseResultRow(row: string): ParsedRow | null {
  ...
  while ((usernameMatch = usernameRegex.exec(playersSection)) !== null) {
    ...
    if (!isInsideMention) {
      ...
      logger.debug(
        `Found plain username: @${usernameMatch[1]} (needs resolution)`
      )
    }
  }
 ...
 * Returns unresolved results that need username/ID resolution
 */
export function parseWordleMessage(
  message: Message
): UnresolvedDailyResults | null {
  const logger = getLogger()
  ...
  for (const result of results) {
    try {
      if (result.discordId) {
        ...
        let matchingMember = guildMembers.find(
          (member) =>
        ...
        if (!matchingMember) {
          matchingMember = guildMembers.find(
            (member) =>
        ...
        if (matchingMember) {
          resolvedResults.push({
            ...
            username:
              ...
              matchingMember.user.displayName ||
              matchingMember.user.username,
            guessCount: result.guessCount,
            hasCrown: result.hasCrown
          })
          logger.debug(
            `Resolved username @${result.username} to ${
              matchingMember.displayName ||
              matchingMember.user.displayName ||
              matchingMember.user.username
            } (${matchingMember.id})`
          ...
          logger.warn(
            `Could not find guild member matching username: @${result.username}`
          )
      ...
      logger.warn(
        `Could not resolve user ${result.discordId || result.username}`,
        error
      )
...
export async function parseAndResolveWordleMessage(
  ...
  if (resolvedResults.length < parsed.results.length) {
    logger.warn(
      `${
        parsed.results.length - resolvedResults.length
