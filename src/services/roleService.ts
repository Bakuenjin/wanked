...
export async function updateRoles(
  ...
  try {
    ...
    logger.info(
      `Role updates complete. Highest: ${
        highestAssigned.join(', ') || 'none'
      }, Lowest: ${lowestAssigned.join(', ') || 'none'}`
    )
    ...
    for (const [memberId, member] of membersWithRole) {
      if (!newOwnerIdSet.has(memberId)) {
        try {
          ...
          logger.debug(
            `Removed ${roleType} ELO role from ${member.user.username}`
          )
        } catch (error) {
          logger.warn(
            `Could not remove role from ${member.user.username}: ${error}`
          )
        }
      }
    }
    ...
    for (const newOwnerId of newOwnerIds) {
      try {
        ...
        if (!newOwner.roles.cache.has(role.id)) {
          ...
          logger.info(
            `Assigned ${roleType} ELO role to ${newOwner.user.username}`
          )
        }
