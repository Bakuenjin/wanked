...
export function calculatePairwiseElo(
  ...
  for (let i = 0; i < participants.length; i++) {
    for (let j = i + 1; j < participants.length; j++) {
      ...
      const expected1 = calculateExpectedScore(
        player1.currentElo,
        player2.currentElo
      )
      const expected2 = calculateExpectedScore(
        player2.currentElo,
        player1.currentElo
      )
      ...
      eloChanges.set(
        player1.playerId,
        (eloChanges.get(player1.playerId) || 0) + change1
      )
      eloChanges.set(
        player2.playerId,
        (eloChanges.get(player2.playerId) || 0) + change2
      )
    }
  }
...
export function processDailyResults(
  ...
  for (const result of results) {
    ...
    if (hasGameRecord(player.id, gameDate)) {
      logger.warn(
        `Player ${result.username} already has a record for ${gameDate}, skipping`
      )
      continue
    }
  ...
  for (const participant of participants) {
    ...
    logger.debug(
      `Player ${participant.playerId}: ${playerData.guessCount}/6, ELO ${
        participant.currentElo
      } -> ${newElo} (${eloChange >= 0 ? '+' : ''}${eloChange})`
    )
  }

  logger.info(
    `Processed ${calculationResults.length} ELO calculations for ${gameDate}`
  )
  return calculationResults
}
