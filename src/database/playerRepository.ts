...
export function getAllPlayersByElo(): Player[] {
  const db = getDatabase()
  const rows = db
    .prepare('SELECT * FROM players ORDER BY elo DESC')
...
export function getActivePlayersByElo(): Player[] {
  const db = getDatabase()
  const rows = db
    .prepare('SELECT * FROM players WHERE is_active = 1 ORDER BY elo DESC')
    .all() as Record<string, unknown>[]
  return rows.map(rowToPlayer)
}
...
export function getHighestEloPlayer(): Player | null {
  ...
  const row = db
    .prepare(
      'SELECT * FROM players WHERE is_active = 1 ORDER BY elo DESC LIMIT 1'
    )
...
export function getTotalPlayerCount(): number {
  const db = getDatabase()
  const result = db.prepare('SELECT COUNT(*) as count FROM players').get() as {
    count: number
  }
  return result.count
}
 ...
 */
export function hasPlayerPlayedOnDate(
  playerId: number,
  gameDate: string
): boolean {
  const db = getDatabase()
