...
export function recordGame(
  ...
  logger.debug(
    `Recorded game for player ${playerId}: ${guessCount}/6, ELO ${eloBefore} -> ${eloAfter}`
  )
 ...
 */
export function getRecentGamesForPlayer(
  playerId: number,
  limit: number = 5
): RecentGame[] {
  const db = getDatabase()
 ...
 */
export function recordEloHistory(
  playerId: number,
  elo: number,
  gameDate: string
): void {
...
export function findNextUnprocessedDate(gameDate: string): string | null {
  let attempts = 0
  const maxAttempts = 7 // Don't look forward more than a week

  while (isDailyResultsProcessed(gameDate) && attempts < maxAttempts) {
  ...
  return attempts < maxAttempts ? gameDate : null
}
